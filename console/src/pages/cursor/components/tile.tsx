    ...
    <>
      <div className="grid grid-cols-3 gap-x-4 gap-y-4">
        {currentList.map(item => (
          <div key={item.course_id} className="cursor-item border border-gray-200 p-4 rounded-lg hover:shadow-md transition-shadow">
            ...
            <p className="font-[600] mt-4 text-lg">{item.course_name}</p>
            <p className="text-sm mt-2 text-gray-400">讲师：{item.instructor}</p>
            <div className="flex justify-between text-sm mt-2 mb-3">
              <CursorStatus status={item.status}></CursorStatus>
              <span className="text-gray-400">{item.enrollment}名学员</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-500">{item.create_time}</span>
              <CourseActions status={item.status} />
            </div>
          </div>
        ))}
      </div>
