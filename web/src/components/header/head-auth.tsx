  ...
  return (
    <Form className="!mt-2" form={loginForm} onFinish={onFinish}>
      ...
      <FormItem name="password" rules={[
        ...
        <Password placeholder="请输入密码" prefix={<LockOutlined />} visibilityToggle={{ visible: passwordVisible, onVisibleChange: setPasswordVisible }}></Password>
      </FormItem>
      <FormItem>
        <Button className="w-full" htmlType="submit" type="primary" disabled={pending} loading={pending}>登陆</Button>
      </FormItem>
    </Form>
  )
}
...
export default function HeadAuth() {
  ...
  return (
    <>
      ...
      || <Space>
        <Spin spinning={pending}>
          <Button type="primary" onClick={() => setOpen(1)}>登录</Button>
        </Spin>
      </Space>}
      <Modal
        open={open !== 0}
